...
def _scipy_exports_blas_int() -> bool:
    """
    Build-time check: does the installed SciPy expose ``blas_int`` from
    ``scipy.linalg.cython_blas``? True for ILP64-aware SciPy (>=1.18),
    False for older SciPy whose Cython BLAS ABI is always LP64.

    Primary path is a Cython compile probe (SciPy's recommended build-time check);
    if that cannot run, fall back to scanning the shipped .pxd files.
    """
    probe = "from scipy.linalg.cython_blas cimport blas_int\n"
    try:
        from Cython.Build import cythonize
        from Cython.Compiler.Errors import CompileError
    except Exception:
        return _scan_pxd_for_blas_int()

    with tempfile.TemporaryDirectory() as tmp:
        pyx = os.path.join(tmp, "_probe_blas_int.pyx")
        with open(pyx, "w") as fh:
            fh.write(probe)
        try:
            # Cython resolves the cimport against scipy's installed 
            # cython_blas.pxd via sys.path; no include_path needed.
            with contextlib.redirect_stderr(io.StringIO()):
                cythonize([pyx], quiet=True, force=True)
        except CompileError:
            return False # symbol absent -> LP64-only SciPy
        except Exception:
            return _scan_pxd_for_blas_int() # unrelated failure -> don't guess
    return True

def _scan_pxd_for_blas_int():
    """Lightweight fallback: grep scipy's shipped cython_blas.pxd."""
    try:
        import scipy.linalg as sla
        pxd = os.path.join(os.path.dirname(sla.__file__), "cython_blas.pxd")
        with open(pxd) as fh:
            return "blas_int" in fh.read()
    except Exception:
        return False

def _create_blas_int_pxd(options):
    """
    Write ``qutip/core/data/_blas_int.pxd`` defining ``blas_int``/``blas_bint``.

    On ILP64-aware SciPy these are re-exported from cython_blas and thus track
    the SciPy build's integer width; on older SciPy we fall back to LP64.
    NOTE: this is the *BLAS ABI* integer, independent of QuTiP's ``idxint``.
    """
    has = _scipy_exports_blas_int()
    import scipy
    print(f"[qutip-build] scipy {scipy.__version__}: blas_int detected = {has}",
          flush=True)
    if has:
        body = (
            "# AUTOGENERATED by setup.py -- do not edit.\n"
            "from scipy.linalg.cython_blas cimport blas_int, blas_bint\n"
        )
    else:
        body = (
            "# AUTOGENERATED by setup.py -- do not edit.\n"
    ...
    path = os.path.join(options['rootdir'], 'qutip', 'core', 'data',
                        '_blas_int.pxd')
